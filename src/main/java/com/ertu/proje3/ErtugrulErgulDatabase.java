...
public class ErtugrulErgulDatabase {
    //Kullanıcı kayıt fonksiyonu
    public static ErtugrulErgulPlayer register(String name, String email, char[] password) {
        try (BufferedReader reader = new BufferedReader(new FileReader("kullanicilar.txt"))) {
            ...
            try (PrintWriter writer = new PrintWriter(new OutputStreamWriter(
                ...
                writer.println(line);
                return new ErtugrulErgulPlayer(name, email, password);
            } catch (IOException ex1) {
                Logger.getLogger(ErtugrulErgulPlayer.class.getName()).log(Level.SEVERE, null, ex1);
            }
            ...
            Logger.getLogger(ErtugrulErgulPlayer.class.getName()).log(Level.SEVERE, null, ex);
        }
        return null;
    }

    public static ErtugrulErgulPlayer login(String email, char[] password) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader("kullanicilar.txt"))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (infos[1].matches(email) && infos[2].matches(String.valueOf(password))) {
                    return new ErtugrulErgulPlayer(infos[0], email, password, infos[3]);
                }
            }
        } catch (IOException ex) {
            Logger.getLogger(ErtugrulErgulPlayer.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public static void registerScore(ErtugrulErgulPlayer player, int score) {
        ...
        try (PrintWriter writer = new PrintWriter(new OutputStreamWriter(
            ...
            try (PrintWriter writer = new PrintWriter(new OutputStreamWriter(
                ...
                Logger.getLogger(ErtugrulErgulDatabase.class.getName()).log(Level.SEVERE, null, ex1);
            }
        }
    }
    
    // All the previous scores are retrieved to create a scoreboard
    public static ArrayList<ErtugrulErgulScore> getScores() {
        ArrayList<ErtugrulErgulScore> scores = new ArrayList<>();
        try (BufferedReader reader = new BufferedReader(new FileReader("skorlar.txt"))) {
            String line;
            while ((line = reader.readLine()) != null) {
                String[] record = line.split(";");
                scores.add(new ErtugrulErgulScore(record[0], Integer.parseInt(record[1]), record[2]));
            }
        } catch (IOException ex) {
            Logger.getLogger(ErtugrulErgulDatabase.class.getName()).log(Level.SEVERE, null, ex);
        }
        // Records are sorted
